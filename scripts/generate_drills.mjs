...
// Helper to determine exact court description based on X/Y
function getPosName(x, isP1) {
  if (isP1) {
    ...
    // Top player
    if (x < X_C - 10) return "Deuce court" // their right
    if (x > X_C + 10) return "Ad court" // their left
    return "Center"
  }
}
...
for (let i = 0; i < 80; i++) {
  ...
  DRILLS.push({
    ...
    diagram_data: {
      ...
      p2: { x: p2_x, y: 70 },
      shots: [
        { x1: p1_x, y1: 840, x2: shot1_x, y2: shot1_y },
        { x1: p2_x, y1: 80, x2: shot2_x, y2: shot2_y }
      ],
      areas: [
        { x: shot1_x, y: shot1_y, r: 25 },
        { x: shot2_x, y: shot2_y, r: 25 }
      ]
    }
  })
}
...
for (let i = 0; i < 50; i++) {
  ...
  const target_x = isDeuce 
    ? (isWide ? rand(X_L, X_L + 40) : rand(X_C - 30, X_C - 5))
    : (isWide ? rand(X_R - 40, X_R) : rand(X_C + 5, X_C + 30))
  
  const target_y = rand(Y_T_SRV + 10, Y_T_SRV + 70) // landing inside box
  ...
  DRILLS.push({
    ...
    diagram_data: {
      p1: { x: p1_x, y: 850 },
      p2: { x: target_x + (isWide ? -20 : 20), y: 70 }, // P2 is returning
      shots: [
        { x1: p1_x, y1: 840, x2: target_x, y2: target_y }
      ],
      areas: [
        { x: target_x, y: target_y, r: 20 }
      ]
    }
  })
}
...
for (let i = 0; i < 50; i++) {
  ...
  DRILLS.push({
    ...
    diagram_data: {
      ...
      p2: { x: coachX, y: 70 },
      shots: [
        { x1: coachX, y1: 80, x2: bounceX, y2: bounceY }, // serve
        { x1: bounceX, y1: bounceY + 20, x2: returnX, y2: returnY } // return
      ],
      moves: [
        { x1: p1X_start, y1: 850, x2: bounceX, y2: bounceY + 40 }
      ],
      areas: [
        { x: returnX, y: returnY, r: 25 }
      ]
    }
  })
}
...
for (let i = 0; i < 50; i++) {
  ...
  const p1_y = rand(Y_NET + 40, Y_B_SRV - 40) // Student at net
  
  const p2_x = rand(X_L + 40, X_R - 40)
  const p2_y = rand(Y_T_BASE, Y_T_SRV - 40) // Coach baseline
  ...
  DRILLS.push({
    ...
    diagram_data: {
      ...
      p2: { x: p2_x, y: p2_y },
      shots: [
        { x1: p2_x, y1: p2_y + 10, x2: p1_x, y2: p1_y - 20 },
        { x1: p1_x, y1: p1_y - 20, x2: volX, y2: volY }
      ],
      areas: [
        { x: volX, y: volY, r: 20 }
      ]
...
for (let i = 0; i < 70; i++) {
  ...
  // Footwork: specific rapid movements
  const run1X = rand(20, X_L)
  const run1Y = rand(800, 850)
  const run2X = rand(X_R, 380)
  const run2Y = rand(800, 850)

  const name = isMatchPlay 
    ? `Point Construction Pattern ${i}` 
    : `Lateral Baseline Sprint ${i}`
    
  const description = isMatchPlay 
    ? `Live ball point play starting with a specific crosscourt feed. Student must sustain depth and wait for a short ball to attack.`
    : `High intensity agility drill. Student starts center, sprints to the outer doubles alley to strike an open-stance groundstroke, and recovers instantly to center.`
  ...
  DRILLS.push({
    ...
    diagram_data: {
      ...
      p2: { x: X_C, y: 70 },
      moves: isMatchPlay ? [] : [
        { x1: p1_x, y1: 840, x2: run1X, y2: run1Y },
        { x1: run1X, y1: run1Y, x2: p1_x, y2: 840 },
        { x1: p1_x, y1: 840, x2: run2X, y2: run2Y }
      ],
      cones: isMatchPlay ? [] : [
        { x: run1X, y: run1Y },
        { x: run2X, y: run2Y }
      ...
      shots: isMatchPlay ? [
        { x1: X_C, y1: 80, x2: X_L + 20, y2: 800 },
        { x1: X_L + 20, y1: 800, x2: X_R - 20, y2: 120 }
      ] : []
    }
  })
}
...
INSERT INTO public.drills (name, description, type, level, group_size, duration_min, diagram_type, diagram_data) VALUES
${DRILLS.map(d => `('${escapeStr(d.name)}', '${escapeStr(d.description)}', '${d.type}', '${d.level}', 'pairs', ${d.duration_min}, 'json', '${JSON.stringify(d.diagram_data).replace(/'/g, "''")}')`).join(',\n')};
`

fs.writeFileSync('scripts/seed_300.sql', sql)
console.log('Saved 300 drills to scripts/seed_300.sql')
